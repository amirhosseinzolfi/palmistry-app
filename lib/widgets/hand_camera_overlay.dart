...
class _HandCameraOverlayState extends State<HandCameraOverlay>
      ...
      builder: (context, child) {
        return CustomPaint(
          painter: _HandOverlayPainter(
            ...
            showVectorHand: widget.showVectorHand,
...
class _HandOverlayPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Rect cropRect =
        Rect.fromCenter(center: center, width: cropW, height: cropH);
    final RRect cropRRect =
        RRect.fromRectAndRadius(cropRect, const Radius.circular(24));
    final Path cutPath = Path()..addRRect(cropRRect);
    final Path vignettePath =
        Path.combine(PathOperation.difference, framePath, cutPath);
    ...
    const double bracketSize = 28.0;
    const double bracketStroke = 3.0;
    final Paint bracketPaint = Paint()
      ..color = isScanning
          ? AppColors.neonElectricBlue
          : AppColors.neonElectricBlue
              .withValues(alpha: 0.80 + pulseValue * 0.20)
    ...
    canvas.drawPath(
      Path()
        ...
        ..arcToPoint(Offset(rrect.left + 12, rrect.top),
    ...
    canvas.drawPath(
      Path()
        ...
        ..arcToPoint(Offset(rrect.right, rrect.top + 12),
            radius: const Radius.circular(12))
    ...
    canvas.drawPath(
      Path()
        ...
        ..arcToPoint(Offset(rrect.left + 12, rrect.bottom),
            radius: const Radius.circular(12))
    ...
    canvas.drawPath(
      Path()
        ...
        ..arcToPoint(Offset(rrect.right, rrect.bottom - 12),
            radius: const Radius.circular(12))
        ..lineTo(rrect.right, rrect.bottom - bracketSize),
      bracketPaint,
    );

    // 3. Render Hand Template Outline Vector only if requested
    if (showVectorHand) {
      canvas.save();
      ...
      final Paint outlinePaint = Paint()
        ..color = isScanning
            ...
            : AppColors.primaryIndigo
                .withValues(alpha: 0.35 + pulseValue * 0.25)
        ..strokeWidth = 2.2
        ..style = PaintingStyle.stroke;

      final Paint fillPaint = Paint()
        ..color =
            AppColors.primaryIndigo.withValues(alpha: 0.04 + pulseValue * 0.03)
    ...
    if (isScanning) {
      final double scanY =
          cropRect.top + (cropH * scanProgress.clamp(0.0, 1.0));

      // Laser line glowing background
      final Paint laserGlowPaint = Paint()
        ..color = AppColors.neonElectricBlue.withValues(alpha: 0.4)
        ..strokeWidth = 6.0
        ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 6.0);
      canvas.drawLine(Offset(cropRect.left + 8, scanY),
          Offset(cropRect.right - 8, scanY), laserGlowPaint);
      ...
      final Paint laserCorePaint = Paint()
        ...
        ..strokeWidth = 2.5
        ..strokeCap = StrokeCap.round;
      canvas.drawLine(Offset(cropRect.left + 12, scanY),
          Offset(cropRect.right - 12, scanY), laserCorePaint);

      // Scanning gradient shadow beam
      final Rect beamRect =
          Rect.fromLTRB(cropRect.left, scanY - 35, cropRect.right, scanY);
      final Paint beamPaint = Paint()
        ..shader = LinearGradient(
          begin: Alignment.topCenter,
          end: Alignment.bottomCenter,
          colors: [
            AppColors.neonElectricBlue.withValues(alpha: 0.0),
            AppColors.neonElectricBlue.withValues(alpha: 0.18),
          ],
  ...
  void _drawGuideLines(Canvas canvas, double pulse, bool scanning) {
    final Paint linePaint = Paint()
      ..color = scanning
          ? AppColors.neonElectricBlue.withValues(alpha: 0.65)
          : AppColors.neonPurple.withValues(alpha: 0.45 + pulse * 0.2)
      ..strokeWidth = 1.8
      ..style = PaintingStyle.stroke
      ..strokeCap = StrokeCap.round;
